  ...
  <div class="jumbotron bg-light vertical-center">
    <!-- first  -->
      ...
      <form>
        ...
        <a class="btn btn-custom-align-center" @click="goToSec"><i class="icon fas fa-chevron-circle-right"></i></a>
      </form>
    </div>
    <!-- first-end -->
     ...
     <div class="second-slide container custom-1 bg-success was-validated bg-secondary text-light p-3" v-bind:class="{'d-none': second}">
      <form>
        ...
        <div class="form-check">
          <input type="checkbox" class="form-check-input" id="agree" v-model="ischecked">
          <label class="form-check-label text-light" for="agree">This is of no use but it looks cool so check this to continue</label>
        </div>
        <div class="btn-banyan-align p-2">
        <a class="btn" @click="goBack"><i  class="fas fa-chevron-circle-left icon"></i></a>
        <a class="btn" @click="validation"><i  class="fas fa-chevron-circle-right icon"></i></a>
        </div>
      </form>
    </div>
  ...
  methods:{
    ...
    validation: function(e){
      ...
      if(this.username.length<4){
        ...
        this.errormsg = 'Thats for show but you still need to check that -_-'
        this.wrong = true
      }else{
        // TODO: THIS IS WHERE IT ENDS FOR NOW
        addUser(this.email,this.password,this.fname,this.lname,this.username,this.$router)
      }
      e.preventDefault()
    }
  }
